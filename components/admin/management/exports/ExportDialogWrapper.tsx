    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogTrigger asChild>
        {trigger || (
          <Button
            variant="outline"
            size="sm"
            className="flex items-center space-x-1 sm:space-x-2 h-8 sm:h-9 px-2 sm:px-3 text-sm sm:text-base border-2 border-primary/20 hover:border-primary/40 hover:bg-primary/5 transition-all duration-200"
          >
      ...
      <DialogContent className="w-[95vw] max-w-[350px] sm:max-w-[500px] md:max-w-[600px] lg:max-w-[700px] max-h-[90vh] sm:max-h-[95vh] overflow-y-auto p-3 sm:p-4 md:p-6">
        <DialogHeader className="pb-2 sm:pb-3 md:pb-4">
          <DialogTitle className="flex items-center space-x-1.5 sm:space-x-2 text-base sm:text-lg md:text-xl">
            <div className="p-1 sm:p-1.5 rounded-lg bg-primary/10">
              <Database className="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5 text-primary" />
            </div>
            <span>{title}</span>
          </DialogTitle>
          <DialogDescription className="text-sm sm:text-base md:text-lg text-muted-foreground text-left">
            {description}
          </DialogDescription>
        </DialogHeader>
        {children}
