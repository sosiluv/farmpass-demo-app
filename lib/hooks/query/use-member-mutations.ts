...
export const useInviteMemberMutation = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      devLog.log("[MUTATION] 멤버 초대 시작", { farmId, email, role });

      const result = await apiClient(`/api/farms/${farmId}/members/invite`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    onSuccess: (_, { farmId }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["farm-members", farmId] });
      queryClient.invalidateQueries({ queryKey: ["farms"] });
      devLog.log("[MUTATION] 멤버 초대 완료, 캐시 무효화");
    },
    onError: (error, { farmId, email, role }) => {
      devLog.error("[MUTATION] 멤버 초대 실패:", error);
      return {
        success: false,
