...
## ✅ PWA 설치 가이드 및 다이얼로그 큐 시스템

- 플랫폼별 PWA 설치 가능 여부를 자동 감지하여, 사용자가 설치 가능한 경우 설치 안내 다이얼로그를 표시합니다.
- iOS, Android, 데스크탑(Chrome/Edge 등) 환경별로 맞춤형 설치 안내를 제공합니다.
- 알림 권한 요청, PWA 설치 안내 등 여러 다이얼로그가 중첩되지 않도록 **다이얼로그 큐 시스템**(Zustand 기반)으로 순차적으로 안내합니다.
- [설치 테스트 페이지](/admin/test-pwa-install)에서 실제 설치 경험을 미리 확인할 수 있습니다.

## ✅ 다크모드 및 테마 시스템

- ShadCN + next-themes 기반의 다크/라이트 테마 전환 기능 제공
- 사이드바 하단의 토글 버튼으로 즉시 테마 변경 가능
- 다크모드에서 텍스트/아이콘 가독성, 버튼/테이블 스타일을 반복적으로 개선하여 모든 화면에서 최적의 시인성 보장

## ✅ 자동 데이터 정리 및 운영 자동화

- 방문자 데이터, 시스템 로그, 푸시 구독 등은 **pg_cron** 기반 SQL 함수로 자동 정리
- 데이터 보존 기간, 정리 스케줄, 수동/자동 실행 현황은 관리자 패널에서 실시간 확인 가능
- 운영자는 관리 패널에서 정리 현황, 예상 정리량, 권장사항을 주간 리포트로 확인

## ✅ 디버그 패널 및 실시간 운영 지원

- 운영자/개발자는 디버그 패널을 통해 실시간 시스템 상태, 로그, 성능, 네트워크 상태를 한눈에 확인할 수 있습니다.
